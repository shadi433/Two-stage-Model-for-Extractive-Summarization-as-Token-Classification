from transformers import (DistilBertTokenizerFast, 
                          DistilBertForTokenClassification, Trainer, TrainingArguments, EarlyStoppingCallback)
import pandas as pd
...
train_labels = [[0 if token == 121 else 1 if token == 122 else token for token in labels] for labels in train_labels]
val_labels = lable_correction(val_labels, val_encodings)
val_labels = [[0 if token == 121 else 1 if token == 122 else token for token in labels] for labels in val_labels]
...
training_args = TrainingArguments(
    output_dir='./distbert_results',          # output directory
    ...
    logging_dir='./distlogs',            # directory for storing logs
...
trainer.save_model("fine_tuned_distbert")
...
latest_checkpoint = find_latest_checkpoint("./distbert_results/")
