from transformers import (BertTokenizerFast, 
                          BertForTokenClassification, Trainer, TrainingArguments, EarlyStoppingCallback)
import pandas as pd
...
train_labels = [[0 if token == 1014 else 1 if token == 1015 else token for token in labels] for labels in train_labels]
val_labels = lable_correction(val_labels, val_encodings)
val_labels = [[0 if token == 1014 else 1 if token == 1015 else token for token in labels] for labels in val_labels]
...
def write_metrics_to_file(metrics):
    with open("./metrics_bert_bert.txt", "w") as f:
        f.write("Metrics:\n")
...
trainer.save_model("fine_tuned_bert1")
...
latest_checkpoint = find_latest_checkpoint("./bert_results/")
